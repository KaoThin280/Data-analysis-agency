...
with col_chat:
    ...
    if user_prompt := st.chat_input(t("e.g., 'Filter sales > 1000 and save as high_sales'", "VD: 'Lọc doanh thu > 1000 và lưu thành bảng high_sales'")):
        ...
        with chat_container.chat_message("assistant"):
            ...
            # Tạo một vùng trống tạm thời để hiển thị trạng thái ngầm (không lưu vào lịch sử chat)
            status_placeholder = st.empty()
            
            with st.spinner(t("AI đang phân tích và thực thi...", "AI is analyzing and executing...")):
                for attempt in range(MAX_RETRIES):
                    # Chỉ cập nhật dòng trạng thái tạm thời để user biết AI đang làm việc
                    if attempt > 0:
                        status_placeholder.info(t(f"🛠️ Đang tối ưu hóa và sửa lỗi (Lần {attempt + 1}/{MAX_RETRIES})...", 
                                                  f"🛠️ Optimizing and fixing errors (Attempt {attempt + 1}/{MAX_RETRIES})..."))
                    ...
                    if code_match:
                        ...
                        final_code = code # Cập nhật mã code mới nhất
                        
                        # Chạy code ngầm (không in ra giao diện lúc này)
                        execution_output, new_dfs = run_python_code_safely(code)
                        
                        if execution_output and execution_output.startswith("❌"):
                            final_error = execution_output
                            
                            # Phân tích lỗi để tạo prompt cho lượt tiếp theo
                            if "ModuleNotFoundError" in final_error or "ImportError" in final_error:
                            ...
                            # TÌNH HUỐNG THÀNH CÔNG: Xóa trạng thái tạm và in kết quả chính thức
                        ...
                        # AI quyết định chỉ trả lời bằng text (vd: giải thích lỗi thư viện)
                        status_placeholder.empty()
                        final_display = response_text
                ...
                # TÌNH HUỐNG THẤT BẠI HOÀN TOÀN (hết số lần thử)
                if not is_resolved:
                    status_placeholder.empty()
                    st.error(t("❌ AI đã thử phân tích nhiều lần nhưng không thành công. Chi tiết lỗi:", "❌ AI tried multiple times but failed. Final error:"))
                    
                    if final_code: # Cho phép user xem đoạn code cuối cùng gây lỗi
                        with st.expander(t("Mã code gây lỗi", "Error Code"), expanded=False):
                            st.code(final_code, language="python")
                            
                    st.code(final_error, language="text")
                    final_display = f"Đã thử sửa lỗi {MAX_RETRIES} lần nhưng không thành công. Lỗi cuối: {final_error}"

                # LƯU VÀO LỊCH SỬ CHAT DUY NHẤT 1 LẦN (Kết quả cuối)
